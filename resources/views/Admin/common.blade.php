<html>
<div id="translate-button" style="background-color: white;"><img
        src="data:image/png;base64,">
</div>
<head>
    <meta charset="utf-8">
    <title>layuimini-单页版 v1 - 基于Layui的后台管理系统前端模板</title>
    <meta name="keywords" content="layuimini,layui,layui模板,layui后台,后台模板,layui mini">
    <meta name="description"
          content="layuimini基于layui的轻量级前端后台管理框架，最简洁、易用的后台框架模板，面向所有层次的前后端程序,只需提供一个接口就直接初始化整个框架，无需复杂操作。">
    <meta name="renderer" content="webkit">
    <meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
    <meta http-equiv="Access-Control-Allow-Origin" content="*">
    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1">
    <meta name="apple-mobile-web-app-status-bar-style" content="black">
    <meta name="apple-mobile-web-app-capable" content="yes">
    <meta name="format-detection" content="telephone=no">
    <link rel="icon" href="{{asset('admin/images/favicon.ico')}}">
    <link rel="stylesheet" href="{{asset('admin/lib/layui-v2.5.5/css/layui.css')}}" media="all">
    <link rel="stylesheet" href="{{asset('admin/css/layuimini.css')}}" media="all">
    <link rel="stylesheet" href="{{asset('admin/css/public.css')}}" media="all">
    <link rel="stylesheet" href="{{asset('admin/lib/font-awesome-4.7.0/css/font-awesome.min.css')}}" media="all">
    <!--[if lt IE 9]>
    <script src="https://cdn.staticfile.org/html5shiv/r29/html5.min.js')}}"></script>
    <script src="https://cdn.staticfile.org/respond.js/1.4.2/respond.min.js')}}"></script>
    <![endif]-->
    <style id="layuimini-bg-color">.layui-layout-admin .layui-header {
            background-color: #1aa094 !important;
        }
        .layui-nav .layui-nav-item a:hover, .layui-nav .layui-this a {
            color: #fff;
        }
        .layui-nav .layui-nav-item a {
            display: block;
            padding: 0 20px;
            color: #fff;
            color: rgba(255,255,255,.7);
            transition: all .3s;
            -webkit-transition: all .3s;
        }
        .layui-header a:hover {
            transition: all .5s;
            -webkit-transition: all .5s;
        }
        a:hover {
            color: #777;
        }
        a:active, a:hover {
            outline: 0;
        }
        .layui-nav * {
            font-size: 14px;
        }
        a {
            color: #333;
            text-decoration: none;
        }
        .layui-header > ul > .layui-nav-item.layui-this, .layuimini-tool i:hover {
            background-color: #197971 !important;
        }

        .layui-layout-admin .layui-logo {
            background-color: #243346 !important;
        }

        .layui-side.layui-bg-black, .layui-side.layui-bg-black > .layui-left-menu > ul {
            background-color: #2f4056 !important;
        }

        .layui-left-menu .layui-nav .layui-nav-child a:hover:not(.layui-this) {
            background-color: #3b3f4b;
        }

        .layui-layout-admin .layui-nav-tree .layui-this, .layui-layout-admin .layui-nav-tree .layui-this > a, .layui-layout-admin .layui-nav-tree .layui-nav-child dd.layui-this, .layui-layout-admin .layui-nav-tree .layui-nav-child dd.layui-this a {
            background-color: #1aa094 !important;
        }</style>
    <style>.one-pan-tip {
            cursor: pointer;
        }

        .one-pan-tip::before {
            background-position: center;
            background-size: 100% 100%;
            background-repeat: no-repeat;
            box-sizing: border-box;
            width: 1em;
            height: 1em;
            margin: 0 1px .15em 1px;
            vertical-align: middle;
            display: inline-block;
        }

        .one-pan-tip-success::before {
            content: '';
            background-image: url(data:image/png;base64,)
        }

        .one-pan-tip-error {
            text-decoration: line-through;
        }

        .one-pan-tip-error::before {
            content: '';
            background-image: url(data:image/png;base64,)
        }

        .one-pan-tip-other::before {
            content: '';
            background-image: url(data:image/png;base64,)
        }

        .one-pan-tip-lock::before {
            content: '';
            background-image: url(data:image/png;base64,)
        }</style>
    <meta name="ljjc::status" content="on">
    <link id="layuicss-layer" rel="stylesheet"
          href="http://localhost.cc/layuimini-onepage/lib/layui-v2.5.5/css/modules/layer/default/layer.css?v=3.1.1"
          media="all">
</head>
<body class="layui-layout-body layuimini-all">
<div class="layui-layout layui-layout-admin">

    <div class="layui-header header">
        <div class="layui-logo"><a href="" one-link-mark="yes">
                <img src="{{asset('admin/images/logo.png')}}" alt="logo">
                <h1>LAYUI MINI</h1>
            </a></div>
        <a one-link-mark="yes">
            <div class="layuimini-tool"><i title="展开" class="fa fa-outdent" data-side-fold="1"></i></div>
        </a>

        <ul class="layui-nav layui-layout-left layui-header-menu layui-header-pc-menu mobile layui-hide-xs">
            <li class="layui-nav-item layui-this" id="currencyHeaderId" data-menu="currency"><a href="javascript:;"
                                                                                                one-link-mark="yes"><i
                        class="fa fa-address-book"></i> 常规管理</a></li>
            <li class="layui-nav-item " id="componentHeaderId" data-menu="component"><a href="javascript:;"
                                                                                        one-link-mark="yes"><i
                        class="fa fa-lemon-o"></i> 组件管理</a></li>
            <li class="layui-nav-item " id="otherHeaderId" data-menu="other"><a href="javascript:;" one-link-mark="yes"><i
                        class="fa fa-slideshare"></i> 其它管理</a></li>
            <span class="layui-nav-bar"></span></ul>
        <ul class="layui-nav layui-layout-left layui-header-menu mobile layui-hide-sm">
            <li class="layui-nav-item">
                <a href="javascript:;" one-link-mark="yes"><i class="fa fa-list-ul"></i> 选择模块<span
                        class="layui-nav-more"></span></a>
                <dl class="layui-nav-child layui-header-mini-menu">
                    <dd><a href="javascript:;" id="currencyHeaderId" data-menu="currency" one-link-mark="yes"><i
                                class="fa fa-address-book"></i> 常规管理</a></dd>
                    <dd><a href="javascript:;" id="componentHeaderId" data-menu="component" one-link-mark="yes"><i
                                class="fa fa-lemon-o"></i> 组件管理</a></dd>
                    <dd><a href="javascript:;" id="otherHeaderId" data-menu="other" one-link-mark="yes"><i
                                class="fa fa-slideshare"></i> 其它管理</a></dd>
                </dl>
            </li>
            <span class="layui-nav-bar"></span></ul>

        <ul class="layui-nav layui-layout-right">
{{--            <li class="layui-nav-item" lay-unselect="">--}}
{{--                <a href="javascript:;" data-refresh="刷新" one-link-mark="yes"><i class="fa fa-refresh"></i></a>--}}
{{--            </li>--}}
{{--            <li class="layui-nav-item" lay-unselect="">--}}
{{--                <a href="javascript:;" data-clear="清理" class="layuimini-clear" one-link-mark="yes"--}}
{{--                   data-href="api/clear.json"><i class="fa fa-trash-o"></i></a>--}}
{{--            </li>--}}
{{--            <li class="layui-nav-item mobile layui-hide-xs" lay-unselect="">--}}
{{--                <a href="javascript:;" data-check-screen="full" one-link-mark="yes"><i class="fa fa-arrows-alt"></i></a>--}}
{{--            </li>--}}
            <li class="layui-nav-item layuimini-setting">
                <a href="javascript:;" one-link-mark="yes">admin<span class="layui-nav-more"></span></a>
                <dl class="layui-nav-child">
                    <dd>
                        <a href="javascript:;" data-content-href="page/user-setting.html" data-title="基本资料"
                           data-icon="fa fa-gears" one-link-mark="yes">基本资料</a>
                    </dd>
                    <dd>
                        <a href="javascript:;" data-content-href="page/user-password.html" data-title="修改密码"
                           data-icon="fa fa-gears" one-link-mark="yes">修改密码</a>
                    </dd>
                    <dd>
                        <a href="javascript:;" class="login-out" one-link-mark="yes">退出登录</a>
                    </dd>
                </dl>
            </li>
{{--            <li class="layui-nav-item layuimini-select-bgcolor mobile layui-hide-xs">--}}
{{--                <a href="javascript:;" data-bgcolor="配色方案" one-link-mark="yes"><i class="fa fa-ellipsis-v"></i></a>--}}
{{--            </li>--}}
{{--            <span class="layui-nav-bar"></span>--}}

        </ul>
    </div>

    <div class="layui-side layui-bg-black">
        <div class="layui-side-scroll layui-left-menu">
            <ul class="layui-nav layui-nav-tree layui-left-nav-tree layui-this" id="currency">
                <li class="layui-nav-item">
                    <a href="javascript:;" class="layui-menu-tips" one-link-mark="yes"><i class="fa fa-home"></i><span
                            class="layui-left-nav"> 主页模板</span> <span class="layui-nav-more"></span></a>
                    <dl class="layui-nav-child">
                        <dd>
                            <a href="javascript:;" class="layui-menu-tips" data-type="tabAdd" data-mpi="m-p-i-1"
                               data-one-page="page/welcome-1.html" target="_self" one-link-mark="yes"><i
                                    class="fa fa-tachometer"></i><span class="layui-left-nav"> 主页一</span></a>
                        </dd>
                        <dd>
                            <a href="javascript:;" class="layui-menu-tips" data-type="tabAdd" data-mpi="m-p-i-2"
                               data-one-page="page/welcome-2.html" target="_self" one-link-mark="yes"><i
                                    class="fa fa-tachometer"></i><span class="layui-left-nav"> 主页二</span></a>
                        </dd>
                        <dd>
                            <a href="javascript:;" class="layui-menu-tips" data-type="tabAdd" data-mpi="m-p-i-3"
                               data-one-page="page/welcome-3.html" target="_self" one-link-mark="yes"><i
                                    class="fa fa-tachometer"></i><span class="layui-left-nav"> 主页三</span></a>
                        </dd>
                    </dl>
                </li>
                <li class="layui-nav-item">
                    <a href="javascript:;" class="layui-menu-tips" one-link-mark="yes"><i class="fa fa-home"></i><span
                            class="layui-left-nav"> 群聊配置</span> <span class="layui-nav-more"></span></a>
                    <dl class="layui-nav-child">
                        <dd>
                            <a href="{{url('/server/pubkey')}}" class="layui-menu-tips" data-type="tabAdd" data-mpi="m-p-i-1"
                               data-one-page="page/welcome-1.html" target="_self" one-link-mark="yes"><i
                                    class="fa fa-tachometer"></i><span class="layui-left-nav"> 关键词列表</span></a>
                        </dd>

                    </dl>
                </li>
                <li class="layui-nav-item">
                    <a href="javascript:;" class="layui-menu-tips" one-link-mark="yes"><i class="fa fa-home"></i><span
                            class="layui-left-nav"> 私聊配置</span> <span class="layui-nav-more"></span></a>
                    <dl class="layui-nav-child">
                        <dd>
                            <a href="{{url('/server/prikey')}}" class="layui-menu-tips" data-type="tabAdd" data-mpi="m-p-i-1"
                               data-one-page="page/welcome-1.html" target="_self" one-link-mark="yes"><i
                                    class="fa fa-tachometer"></i><span class="layui-left-nav"> 关键词列表</span></a>
                        </dd>

                    </dl>
                </li>
                <li class="layui-nav-item">
                    <a href="javascript:;" class="layui-menu-tips" data-type="tabAdd" data-mpi="m-p-i-4"
                       data-one-page="page/menu.html" target="_self" one-link-mark="yes"><i
                            class="fa fa-window-maximize"></i><span class="layui-left-nav"> 菜单管理</span></a>
                </li>
                <li class="layui-nav-item">
                    <a href="javascript:;" class="layui-menu-tips" data-type="tabAdd" data-mpi="m-p-i-5"
                       data-one-page="page/setting.html" target="_self" one-link-mark="yes"><i
                            class="fa fa-gears"></i><span class="layui-left-nav"> 系统设置</span></a>
                </li>
                <li class="layui-nav-item">
                    <a href="javascript:;" class="layui-menu-tips" data-type="tabAdd" data-mpi="m-p-i-6"
                       data-one-page="page/table.html" target="_self" one-link-mark="yes"><i
                            class="fa fa-file-text"></i><span class="layui-left-nav"> 表格示例</span></a>
                </li>
                <li class="layui-nav-item">
                    <a href="javascript:;" class="layui-menu-tips" one-link-mark="yes"><i
                            class="fa fa-calendar"></i><span class="layui-left-nav"> 表单示例</span> <span
                            class="layui-nav-more"></span></a>
                    <dl class="layui-nav-child">
                        <dd>
                            <a href="javascript:;" class="layui-menu-tips" data-type="tabAdd" data-mpi="m-p-i-7"
                               data-one-page="page/form.html" target="_self" one-link-mark="yes"><i
                                    class="fa fa-list-alt"></i><span class="layui-left-nav"> 普通表单</span></a>
                        </dd>
                        <dd>
                            <a href="javascript:;" class="layui-menu-tips" data-type="tabAdd" data-mpi="m-p-i-8"
                               data-one-page="page/form-step.html" target="_self" one-link-mark="yes"><i
                                    class="fa fa-navicon"></i><span class="layui-left-nav"> 分步表单</span></a>
                        </dd>
                    </dl>
                </li>
                <li class="layui-nav-item">
                    <a href="javascript:;" class="layui-menu-tips" one-link-mark="yes"><i class="fa fa-flag-o"></i><span
                            class="layui-left-nav"> 登录模板</span> <span class="layui-nav-more"></span></a>
                    <dl class="layui-nav-child">
                        <dd>
                            <a href="javascript:;" class="layui-menu-tips" data-type="tabAdd" data-mpi="m-p-i-9"
                               data-one-page="page/login-1.html" target="_blank" one-link-mark="yes"><i
                                    class="fa fa-stumbleupon-circle"></i><span class="layui-left-nav"> 登录-1</span></a>
                        </dd>
                        <dd>
                            <a href="javascript:;" class="layui-menu-tips" data-type="tabAdd" data-mpi="m-p-i-10"
                               data-one-page="page/login-2.html" target="_blank" one-link-mark="yes"><i
                                    class="fa fa-viacoin"></i><span class="layui-left-nav"> 登录-2</span></a>
                        </dd>
                    </dl>
                </li>
                <li class="layui-nav-item">
                    <a href="javascript:;" class="layui-menu-tips" one-link-mark="yes"><i class="fa fa-home"></i><span
                            class="layui-left-nav"> 异常页面</span> <span class="layui-nav-more"></span></a>
                    <dl class="layui-nav-child">
                        <dd>
                            <a href="javascript:;" class="layui-menu-tips" data-type="tabAdd" data-mpi="m-p-i-11"
                               data-one-page="page/404.html" target="_self" one-link-mark="yes"><i
                                    class="fa fa-hourglass-end"></i><span class="layui-left-nav"> 404页面</span></a>
                        </dd>
                    </dl>
                </li>
                <li class="layui-nav-item">
                    <a href="javascript:;" class="layui-menu-tips" one-link-mark="yes"><i class="fa fa-snowflake-o"></i><span
                            class="layui-left-nav"> 其它界面</span> <span class="layui-nav-more"></span></a>
                    <dl class="layui-nav-child">
                        <dd>
                            <a href="javascript:;" class="layui-menu-tips" data-type="tabAdd" data-mpi="m-p-i-12"
                               data-one-page="page/button.html" target="_self" one-link-mark="yes"><i
                                    class="fa fa-snowflake-o"></i><span class="layui-left-nav"> 按钮示例</span></a>
                        </dd>
                        <dd>
                            <a href="javascript:;" class="layui-menu-tips" data-type="tabAdd" data-mpi="m-p-i-13"
                               data-one-page="page/layer.html" target="_self" one-link-mark="yes"><i
                                    class="fa fa-shield"></i><span class="layui-left-nav"> 弹出层</span></a>
                        </dd>
                    </dl>
                </li>
                <span class="layui-nav-bar" style="top: 101px; height: 0px; opacity: 0;"></span></ul>
            <ul class="layui-nav layui-nav-tree layui-left-nav-tree layui-hide" id="component">
                <li class="layui-nav-item">
                    <a href="javascript:;" class="layui-menu-tips" data-type="tabAdd" data-mpi="m-p-i-14"
                       data-one-page="page/icon.html" target="_self" one-link-mark="yes"><i
                            class="fa fa-dot-circle-o"></i><span class="layui-left-nav"> 图标列表</span></a>
                </li>
                <li class="layui-nav-item">
                    <a href="javascript:;" class="layui-menu-tips" data-type="tabAdd" data-mpi="m-p-i-15"
                       data-one-page="page/icon-picker.html" target="_self" one-link-mark="yes"><i
                            class="fa fa-adn"></i><span class="layui-left-nav"> 图标选择</span></a>
                </li>
                <li class="layui-nav-item">
                    <a href="javascript:;" class="layui-menu-tips" data-type="tabAdd" data-mpi="m-p-i-16"
                       data-one-page="page/color-select.html" target="_self" one-link-mark="yes"><i
                            class="fa fa-dashboard"></i><span class="layui-left-nav"> 颜色选择</span></a>
                </li>
                <li class="layui-nav-item">
                    <a href="javascript:;" class="layui-menu-tips" data-type="tabAdd" data-mpi="m-p-i-17"
                       data-one-page="page/table-select.html" target="_self" one-link-mark="yes"><i
                            class="fa fa-angle-double-down"></i><span class="layui-left-nav"> 下拉选择</span></a>
                </li>
                <li class="layui-nav-item">
                    <a href="javascript:;" class="layui-menu-tips" data-type="tabAdd" data-mpi="m-p-i-18"
                       data-one-page="page/upload.html" target="_self" one-link-mark="yes"><i
                            class="fa fa-arrow-up"></i><span class="layui-left-nav"> 文件上传</span></a>
                </li>
                <li class="layui-nav-item">
                    <a href="javascript:;" class="layui-menu-tips" data-type="tabAdd" data-mpi="m-p-i-19"
                       data-one-page="page/editor.html" target="_self" one-link-mark="yes"><i
                            class="fa fa-edit"></i><span class="layui-left-nav"> 富文本编辑器</span></a>
                </li>
                <li class="layui-nav-item">
                    <a href="javascript:;" class="layui-menu-tips" data-type="tabAdd" data-mpi="m-p-i-20"
                       data-one-page="page/area.html" target="_self" one-link-mark="yes"><i
                            class="fa fa-rocket"></i><span class="layui-left-nav"> 省市县区选择器</span></a>
                </li>
                <span class="layui-nav-bar"></span></ul>
            <ul class="layui-nav layui-nav-tree layui-left-nav-tree layui-hide" id="other">
                <li class="layui-nav-item">
                    <a href="javascript:;" class="layui-menu-tips" one-link-mark="yes"><i class="fa fa-meetup"></i><span
                            class="layui-left-nav"> 多级菜单</span> <span class="layui-nav-more"></span></a>
                    <dl class="layui-nav-child">
                        <dd>
                            <a href="javascript:;" class="layui-menu-tips" one-link-mark="yes"><i
                                    class="fa fa-calendar"></i><span class="layui-left-nav"> 按钮1</span><span
                                    class="layui-nav-more"></span></a>
                            <dl class="layui-nav-child">
                                <dd>
                                    <a href="javascript:;" class="layui-menu-tips" one-link-mark="yes"><i
                                            class="fa fa-snowflake-o"></i><span class="layui-left-nav"> 按钮2</span><span
                                            class="layui-nav-more"></span></a>
                                    <dl class="layui-nav-child">
                                        <dd>
                                            <a href="javascript:;" class="layui-menu-tips" data-type="tabAdd"
                                               data-mpi="m-p-i-21" data-one-page="page/button-1.html" target="_self"
                                               one-link-mark="yes"><i class="fa fa-snowflake-o"></i><span
                                                    class="layui-left-nav"> 三级菜单1</span></a>
                                        </dd>
                                        <dd>
                                            <a href="javascript:;" class="layui-menu-tips" data-type="tabAdd"
                                               data-mpi="m-p-i-22" data-one-page="page/form-1.html" target="_self"
                                               one-link-mark="yes"><i class="fa fa-calendar"></i><span
                                                    class="layui-left-nav"> 三级菜单2</span></a>
                                        </dd>
                                    </dl>
                                </dd>
                            </dl>
                        </dd>
                    </dl>
                </li>
                <li class="layui-nav-item">
                    <a href="javascript:;" class="layui-menu-tips" data-type="tabAdd" data-mpi="m-p-i-23"
                       data-one-page="page/error.html" target="_self" one-link-mark="yes"><i
                            class="fa fa-superpowers"></i><span class="layui-left-nav"> 失效菜单</span></a>
                </li>
                <span class="layui-nav-bar"></span></ul>
        </div>
    </div>

    <div class="layui-body">
        @section("content")
        @show
    </div>

</div>


<script src="{{asset('admin/lib/layui-v2.5.5/layui.js?v=1.0.4')}}" charset="utf-8"></script>
<script src="{{asset('admin/js/lay-config.js?v=1.0.4')}}" charset="utf-8"></script>
<script>
    layui.use(['element', 'layer', 'layuimini'], function () {
        var $ = layui.jquery,
            element = layui.element,
            layer = layui.layer,
            layuimini = layui.layuimini;

        layuimini.init('api/init.json');

        $('.login-out').on("click", function () {
            layer.msg('退出登录成功', function () {
                window.location = '/page/login-1.html';
            });
        });

        layuimini.listen();

    });
</script>


<div class="layui-layer-move"></div>
</body>
</html>
